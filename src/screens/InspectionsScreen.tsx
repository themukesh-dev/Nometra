...
type BackendInspection = {
  ...
  timestamp?: string;
  created_at?: string;
  overall_status?: 'COMPLIANT' | 'NON_COMPLIANT' | 'VERIFICATION_REQUIRED';
...
function normalizeInspection(
  ...
  const report = inspection.compliance_report;
  ...
  return {
    ...
    status: normalizeStatus(inspection),
    passed:
      typeof inspection.passed === 'number'
        ? inspection.passed
        : typeof report?.passed === 'number'
        ? report.passed
        : 0,
    failed:
      typeof inspection.failed === 'number'
        ? inspection.failed
...
export default function InspectionsScreen() {
  ...
  const loadInspections = async () => {
    ...
    try {
       ...
       * This also safely handles { inspections: [...] }
      ...
      const rows = backendInspections.map(
        normalizeInspection
      );

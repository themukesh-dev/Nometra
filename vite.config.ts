...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      host:
        process.env.FIGMA_DEV_SERVER_HOST ||
        '0.0.0.0',
      ...
      //
      // The /api prefix is removed because Flask exposes:
      //     @app.route("/scan")
      // --------------------------------------------------
      proxy: {
        '/api': {
...
function figmaErrorOverlayReplay(): Plugin {
  return {
    name:
      'figma-error-overlay-replay',

    apply: 'serve',

    configureServer(server) {
      let lastError: object | null =
        null
...
/**
 * Reload when a module that previously defined a React Refresh boundary
 * stops defining one.
 */
function figmaReactRefreshBoundaryFallback(): Plugin {
  const hadRefreshBoundary =
  ...
  return {
    name:
      'figma-react-refresh-boundary-fallback',
 ...
 */
function figmaMakeKitPlugin(
  options: {
    storiesGlob: string | string[]
  },
): Plugin {
  const storiesGlob =
    Array.isArray(options.storiesGlob)
      ? options.storiesGlob
      : [options.storiesGlob]
  ...
  const VIRTUAL_ID =
    'virtual:figma-stories'

  const RESOLVED_ID =
    '\0' + VIRTUAL_ID

  const STORIES_MODULE =
    `export const stories = import.meta.glob(${JSON.stringify(
      storiesGlob,
    )})`
